...
class TodoDataUtils {
  ...
  static bool isComplete(List<TodoItem> items, DateTime date) {
  ...
  static List<TodoItem> findCompleteListByDateTime(List<TodoItem> items, DateTime date) {
    var cItems = findTodoListByDateTime(items, date);
    var values = cItems.where((item) => item.isDone).toList();
    return values;
  }
